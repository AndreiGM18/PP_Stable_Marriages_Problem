...
; întoarce false.
; Folosește find-first, fără să îl apeleze de 2 ori (hint: define în define).
...
(define (stable-match? engagements mpref wpref)
  (null? (filter
          (λ (pair) (or
                     (better-match-exists? (car pair) (cdr pair) (get-pref-list wpref (car pair)) mpref (map (λ (pair) (cons (cdr pair) (car pair))) engagements))
